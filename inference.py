...
class Model:
    ...
    def predict(self, image):
        pixel_values = self.preprocessing(image)
        logits = self.forward_pass(pixel_values)
        height, width, _ = self.get_image_shape(image)
        # First, rescale logits to original image size
        upsampled_logits = nn.functional.interpolate(logits, size=(height, width),  # (height, width)
                                                     mode='bilinear', align_corners=False)
        # Second, apply argmax on the class dimension
        seg = upsampled_logits.argmax(dim=1)[0]

        color_seg = np.zeros_like(image, dtype=np.uint8)  # height, width, 3
        if self.id2color is None:
            id2color, id2name, name2id = self.read_color_map()
    ...
    def inference_image(self, image_path):
        ...
        for image_path in images:
            ...
            if self.record_outputs:
                cv2.imwrite(f"{self.recording_path}/{im_name}.png", output_image)
                counter += 1
...
if __name__ == "__main__":
    ...
    output_path = 'outputs'
    image_path = "inputs/*.png"  # The image-images path for want to inference (RegEx)
